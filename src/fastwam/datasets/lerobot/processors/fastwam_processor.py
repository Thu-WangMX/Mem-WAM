...
class FastWAMProcessor(BaseProcessor):
    ...
    def preprocess(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sample["idx"] = data["idx"]
        # Preserve causal provenance for memory-augmented datasets.  These are
        # scalar metadata only and therefore remain compatible with the
        # default PyTorch collator.
        for key in ("episode_index", "frame_index", "dataset_index"):
            if key in data:
                sample[key] = data[key]
