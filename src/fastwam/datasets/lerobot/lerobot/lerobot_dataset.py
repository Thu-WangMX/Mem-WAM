...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # Check timestamps
        # datasets>=4 returns a lazy Column here; torch.stack only accepts a
        # concrete sequence even though individual items are transformed to
        # tensors.
        timestamps = torch.stack(list(self.hf_dataset["timestamp"])).numpy()
        episode_indices = torch.stack(
            list(self.hf_dataset["episode_index"])
        ).numpy()
    ...
    def get_episode_data(self, episode_id: int) -> dict:
        ...
        res = {
            key: torch.stack(list(selected_data[key])) for key in res_keys
        }
        return res
