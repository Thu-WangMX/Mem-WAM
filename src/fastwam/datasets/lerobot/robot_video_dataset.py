...
class RobotVideoDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        robotwin_head_position: str = "top",
        ...
        self.robotwin_head_position = str(robotwin_head_position).lower()
        if self.robotwin_head_position not in {"top", "bottom"}:
            raise ValueError(
                "`robotwin_head_position` must be 'top' or 'bottom', "
                f"got {robotwin_head_position!r}"
            )
    ...
    def _get(self, idx):
        ...
        if self.concat_multi_camera == "robotwin":
            ...
            if self.robotwin_head_position == "top":
                video = torch.cat([cam_top, bottom], dim=-2)
            else:
                video = torch.cat([bottom, cam_top], dim=-2)
        elif num_cameras > 1:
        ...
        # Keep scalar sample provenance through the final video wrapper.  The
        # official processor already preserved these fields; dropping them
        # here would make causal memory alignment unverifiable.
        for key in ("idx", "episode_index", "frame_index", "dataset_index"):
            if key in sample:
                data[key] = sample[key]
        return data
