...
class WanVideoDiT(torch.nn.Module):
    ...
    def _validate_forward_inputs(
        ...
        if timestep.ndim not in (1, 2):
            raise ValueError(
                "`timestep` must be [B], [1], or per-frame [B,F], "
    ...
    def pre_dit(
        ...
        if self.seperated_timestep and fuse_vae_embedding_in_latents:
            ...
            if timestep.ndim == 2:
                ...
                frame_timesteps = timestep.view(batch_size, 1).expand(
                    -1, x.shape[2]
                ).clone()
                frame_timesteps[:, 0] = 0
        ...
        temporal_position_offset = int(temporal_position_offset)
        if temporal_position_offset < 0:
            raise ValueError(
                "`temporal_position_offset` must be non-negative, "
                f"got {temporal_position_offset}"
            )
        temporal_end = temporal_position_offset + f
        if temporal_end > self.freqs[0].shape[0]:
            raise ValueError(
                "Video temporal positions exceed the RoPE cache: "
                f"offset={temporal_position_offset}, frames={f}, "
                f"capacity={self.freqs[0].shape[0]}"
            )
        freqs = torch.cat([
            self.freqs[0][temporal_position_offset:temporal_end].view(
                f, 1, 1, -1
            ).expand(f, h, w, -1),
