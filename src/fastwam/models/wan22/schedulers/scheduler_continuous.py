...
class WanContinuousFlowMatchScheduler:
    ...
    def __init__(
        ...
        shift: float = 5.0,
        eps: float = 1e-10,
        training_sampling_scheme: str = "uniform",
        logit_mean: float = 0.0,
        logit_std: float = 1.0,
        training_weight_scheme: str = "fastwam",
    ):
        if num_train_timesteps <= 0:
            raise ValueError(f"`num_train_timesteps` must be positive, got {num_train_timesteps}")
        if shift <= 0:
            raise ValueError(f"`shift` must be positive, got {shift}")
        if training_sampling_scheme not in {"uniform", "logit_normal"}:
            raise ValueError(
                "`training_sampling_scheme` must be 'uniform' or "
                f"'logit_normal', got {training_sampling_scheme!r}"
            )
        if logit_std <= 0:
            raise ValueError(f"`logit_std` must be positive, got {logit_std}")
        if training_weight_scheme not in {"fastwam", "none"}:
            raise ValueError(
                "`training_weight_scheme` must be 'fastwam' or 'none', "
                f"got {training_weight_scheme!r}"
            )
        self.num_train_timesteps = int(num_train_timesteps)
        self.shift = float(shift)
        self.eps = float(eps)
        self.training_sampling_scheme = training_sampling_scheme
        self.logit_mean = float(logit_mean)
        self.logit_std = float(logit_std)
        self.training_weight_scheme = training_weight_scheme
    ...
    def sample_training_t(self, batch_size: int, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
        ...
        if self.training_sampling_scheme == "logit_normal":
            logits = torch.randn((batch_size,), device=device, dtype=torch.float32)
            u = torch.sigmoid(logits * self.logit_std + self.logit_mean)
        else:
    ...
    def training_weight(self, timestep: torch.Tensor) -> torch.Tensor:
        ...
        if self.training_weight_scheme == "none":
