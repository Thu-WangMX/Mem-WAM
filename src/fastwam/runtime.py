...
def create_fastwam(
    ...
    model_dtype: torch.dtype = torch.bfloat16,
    device: str = "cuda",
    memory_backend: str = "original",
    helios_config=None,
):
    from .models.wan22.fastwam import FastWAM

    extra_model_args = {}
    if memory_backend == "helios":
        from .models.wan22.fastwam_helios import FastWAMHelios
        FastWAM = FastWAMHelios
        if isinstance(helios_config, DictConfig):
            helios_config = OmegaConf.to_container(helios_config, resolve=True)
        extra_model_args['helios_config'] = helios_config
    elif memory_backend != "original":
    ...
    if isinstance(native_cache, DictConfig):
        native_cache = OmegaConf.to_container(native_cache, resolve=True)
    if native_cache is not None and not isinstance(native_cache, dict):
        raise ValueError(f"`native_cache` must be dict-like, got {type(native_cache)}")
    ...
    return FastWAM.from_wan22_pretrained(
        ...
        video_training_sampling_scheme=str(
            video_scheduler.get("training_sampling_scheme", "uniform")
        ),
        video_logit_mean=float(video_scheduler.get("logit_mean", 0.0)),
        video_logit_std=float(video_scheduler.get("logit_std", 1.0)),
        video_training_weight_scheme=str(
            video_scheduler.get("training_weight_scheme", "fastwam")
        ),
        action_train_shift=float(action_scheduler["train_shift"]),
        action_infer_shift=float(action_scheduler["infer_shift"]),
        action_num_train_timesteps=int(action_scheduler["num_train_timesteps"]),
        action_training_sampling_scheme=str(
            action_scheduler.get("training_sampling_scheme", "uniform")
        ),
        action_logit_mean=float(action_scheduler.get("logit_mean", 0.0)),
        action_logit_std=float(action_scheduler.get("logit_std", 1.0)),
        action_training_weight_scheme=str(
            action_scheduler.get("training_weight_scheme", "fastwam")
        ),
        action_rope_spatial_mode=str(action_rope_spatial_mode),
...
def run_training(cfg: DictConfig):
    ...
    # Model construction contains the deliberately fresh ActionDiT
    # input/output heads and proprio encoder. Seed before construction so
    # their initialization is reproducible and identical on every rank.
