...
LOCK_SCHEMA_V2 = "putback_locked_control_information_candidate_v2"
...
def lock_candidate_v2(
    ...
    *,
    ...
    path = Path(path)
    if path.exists():
        raise FileExistsError(f"refusing to overwrite locked v2 candidate: {path}")
    if set(dependency_paths) != REQUIRED_DEPENDENCY_KEYS_V2:
        raise ValueError(
            "v2 candidate dependencies must be exactly "
            f"{sorted(REQUIRED_DEPENDENCY_KEYS_V2)}"
        )
    payload = {
        ...
        "hashes": {
            ...
            for key in sorted(REQUIRED_DEPENDENCY_KEYS_V2)
        },
    }
...
def _evaluate_candidate_v2(
    *,
    ...
    for episode in episodes:
        ...
        analysis_adapter = CausalEpisodeLogBiasAdapter(
            statistics=statistics,
            calibration_samples=int(config["calibration_samples"]),
            epsilon=float(config["adaptation_epsilon"]),
            max_abs_log_bias=float(config["max_abs_log_bias"]),
        )
        ...
        for frame in detector_frames:
            ...
            if value is not None:
                adapted = analysis_adapter.observe(frame=frame, information=value)
                if adapted is not None:
                    z_by_frame[frame] = contextual_information_z(
                        adapted, frame=frame, statistics=statistics
                    )
        ...
        if state.adaptation_factor is None:
            raise RuntimeError("v2 episode did not freeze its causal adaptation")
        adaptation_factors.append(float(state.adaptation_factor))
        ...
        for row in aligned["confirmation_to_decision"]:
            ...
            if reason == "counterfactual_control_information":
                ...
                forced += 1
        excess = {
            frame: max(0.0, z - float(config["drift"]))
        ...
        episode_reports[str(episode)] = {
            "adaptation_factor": float(state.adaptation_factor),
            "calibration_frames": list(state.adapter.calibration_frames),
...
def main() -> None:
    ...
    if output.exists():
        raise FileExistsError(f"refusing to overwrite v2 selector calibration: {output}")
    ...
    if (
        ...
        raise ValueError("v2 candidate traces must contain exactly episodes 0-39")
    ...
    if temporary.exists():
        raise FileExistsError(f"stale v2 calibration temporary: {temporary}")
        ...
        candidates.append(
            {
                ...
                "metrics": _evaluate_candidate_v2(
                    traces=traces,
    ...
    report = {
        "schema_version": "putback_control_information_calibration_v2",
        "split": split,
        "grid": {
