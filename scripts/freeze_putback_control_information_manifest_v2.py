...
from fastwam.evaluation.control_information_online_v2 import RUNTIME_LOCK_SCHEMA_V2
...
def freeze_episode_v2(
    *,
    ...
    if frames.ndim != 1 or information.shape != frames.shape:
        raise ValueError("v2 counterfactual-control trace shape mismatch")
    information_by_frame = {
        int(frame): float(value)
        for frame, value in zip(frames.tolist(), information.tolist())
    }
    events = replay_control_information_trace_v2(
    ...
    if not events:
        raise RuntimeError("v2 episode did not produce a terminal partition")
    factors = {float(event.adaptation_factor) for event in events}
    if len(factors) != 1:
        raise RuntimeError("v2 episode adaptation changed after calibration")
    ...
    payload = {
        ...
        "adaptation_factor": factors.pop(),
        "calibration_frames": [16, 20, 24, 28],
...
def _validated_trace(path: Path, expected_episodes: list[int]) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError(f"v2 trace {path} does not contain episodes {expected_episodes}")
    return payload
...
def main() -> None:
    ...
    if output.exists():
        raise FileExistsError(f"refusing to overwrite v2 planning manifest: {output}")
    runtime_lock_path = Path(args.runtime_lock).resolve()
    runtime_lock = json.loads(runtime_lock_path.read_text())
    if runtime_lock.get("schema_version") != RUNTIME_LOCK_SCHEMA_V2:
    ...
    if runtime_lock["runtime_hashes"]["offline_freezer_source"] != sha256_file(
        ...
        raise ValueError("v2 offline freezer source differs from runtime lock")
    ...
    if candidate_lock["hashes"]["contextual_statistics"] != sha256_file(
        ...
        raise ValueError("v2 contextual statistics differ from locked candidate")
    ...
    if late["hashes"].get("runtime_lock") != sha256_file(runtime_lock_path):
        raise ValueError("v2 heldout traces were not opened by this runtime lock")

    temporary = output.with_name(output.name + ".tmp")
    if temporary.exists():
        raise FileExistsError(f"stale v2 planning-manifest temporary: {temporary}")
    ...
    for episode in range(50):
        ...
        adaptation_factors.append(float(payload["adaptation_factor"]))
    ...
    metadata = {
        ...
        "replan_stride": 16,
        "detector_stride": 4,
        "selector": "locked_episode_calibrated_control_information_v2",
