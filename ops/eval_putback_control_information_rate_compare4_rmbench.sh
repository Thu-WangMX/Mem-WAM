...
rate_mode="${RATE_MODE:-event}"
case "${rate_mode}" in
  event)
    train_root_default="/mnt/vepfs02/output/kevin.wang/memorywam/train/control_information_v4_event_full_forced_half_putback_e2e_30k_seed42"
    model_config="fastwam_event_conditioned_rate"
    expected_max_steps=30000
    expected_save_every=10000
    expected_save_steps="[5000]"
    expected_memory_tokens=48
    expected_allocation_mode="event_full_forced_half"
    sim_config="sim_robotwin_control_information_rate_event"
    ;;
  k8l)
    train_root_default="/mnt/vepfs02/output/kevin.wang/memorywam/train/control_information_v4_anchor_recent_k8l_putback_e2e_30k_seed42"
    model_config="fastwam_variable_rate_k8l"
    expected_max_steps=30000
    expected_save_every=10000
    expected_save_steps="[5000]"
    expected_memory_tokens=48
    expected_allocation_mode="span_full"
    sim_config="sim_robotwin_control_information_rate_k8l"
    ;;
  *) echo "ERROR: RATE_MODE must be event or k8l" >&2; exit 2 ;;
esac
...
run_tag="${RUN_TAG:-control_information_rate_${rate_mode}_rmbench4_$(date +%Y%m%d_%H%M%S)}"
...
cd "${repo_root}"
preflight_report="$("${python_bin}" - "${checkpoint}" "${manifest_root}" "${runtime_lock}" "${rate_mode}" "${model_config}" "${expected_max_steps}" "${expected_save_every}" "${expected_save_steps}" "${expected_memory_tokens}" "${expected_allocation_mode}" "${sim_config}" <<'PY'
...
rate_mode = sys.argv[4]
model_config = sys.argv[5]
expected_max_steps = int(sys.argv[6])
expected_save_every = int(sys.argv[7])
expected_save_steps = json.loads(sys.argv[8])
expected_memory_tokens = int(sys.argv[9])
expected_allocation_mode = sys.argv[10]
sim_config = sys.argv[11]
...
assert 5000 <= checkpoint_step <= expected_max_steps and checkpoint_step % 5000 == 0
...
expected_training = {
    ...
    "model.native_cache.memory_tokens": expected_memory_tokens,
...
if rate_mode == "event":
    assert OmegaConf.select(cfg, "memory_allocation_rule") == "event_full_forced_half"
else:
    assert OmegaConf.select(cfg, "memory_allocation_rule") is None
...
assert eval_cfg.model.native_cache.memory_tokens == expected_memory_tokens
assert eval_cfg.model.native_cache.allocation_mode == expected_allocation_mode
...
print(f"rate_mode={rate_mode} model={model_config}")
print(f"memory_tokens={expected_memory_tokens} allocation_mode={expected_allocation_mode} detector_stride=4 replan_steps=16")
...
for position in "${!indices[@]}"; do
  ...
  "EVALUATION.task_config=demo_clean" \
    "EVALUATION.policy_name=${POLICY_NAME:-fastwam_rate_compare_policy}" \
...
summary="${log_root}/summary.txt"
{
  ...
  echo "rate_mode=${rate_mode} allocation_mode=${expected_allocation_mode} selector=segment_relative_counterfactual_control_information runtime=v3 initialization_wam=frozen strict_online=true"
  for label in "${labels[@]}"; do
    log="${log_root}/${label}.log"
    result="$(grep -a 'Success rate:' "${log}" | tail -n 1 || true)"
    audit="$("${python_bin}" - "${log}" "${rate_mode}" <<'PY'
from __future__ import annotations
...
for event in events:
    ...
    arrival = matching[-1]
    # Detector events are reported in stride-4 video-frame coordinates, while
    # the layerwise memory plan is indexed by stride-16 decision latents.  The
    # arrival record's half-open close_range is therefore the authoritative
    # span for the token budget (e.g. raw [0,36) -> decision [0,3)).
    close_start, close_stop = (int(value) for value in arrival["close_range"])
    assert close_stop > close_start
    assert close_start == (start + 15) // 16, (start, close_start)
    assert close_stop == (end + 15) // 16, (end, close_stop)
    ...
    assert int(arrival.get("close_token_count", -1)) == expected_tokens, (
        rate_mode, reason, start, end, arrival
    )
    previous_end = end
