...
output_dir="${OUTPUT_DIR:-/mnt/vepfs02/output/kevin_wang/memorywam/train/layerwise_block_memory_battery_e2e_25k_seed42}"
...
export RMBENCH_BATTERY_LEROBOT="${RMBENCH_BATTERY_LEROBOT:-/mnt/vepfs02/output/kevin_wang/memorywam/data/fastwam_fullkv_remaining8_rgb_v3/battery_try/lerobot/battery_try}"
export MEMORYWAM_BATTERY_STATS="${MEMORYWAM_BATTERY_STATS:-/mnt/vepfs02/output/kevin_wang/memorywam/data/fastwam_fullkv_remaining8_rgb_v3/battery_try/stats/dataset_stats.json}"
export MEMORYWAM_BATTERY_TEXT_CACHE="${MEMORYWAM_BATTERY_TEXT_CACHE:-/mnt/vepfs02/output/kevin_wang/memorywam/data/fastwam_fullkv_remaining8_rgb_v3/battery_try/text_cache}"
export MEMORYWAM_BATTERY_CONTINUOUS_LATENTS="${MEMORYWAM_BATTERY_CONTINUOUS_LATENTS:-/mnt/vepfs02/output/kevin_wang/memorywam/data/fastwam_fullkv_remaining8_rgb_v3/battery_try/temporal_fullkv_continuous_episode_stride16_v4}"
...
export NATIVE_CACHE_BATTERY_25K_OUTPUT="${output_dir}"
...
battery_asset_root="${BATTERY_ASSET_ROOT:-$(dirname "$(dirname "${RMBENCH_BATTERY_LEROBOT}")")}" 
build_manifest="${battery_asset_root}/_build_manifest.json"
rgb_contract="${battery_asset_root}/_build_logs/rgb_contract.json"
latent_manifest="${MEMORYWAM_BATTERY_CONTINUOUS_LATENTS}/manifest.json"
...
[[ -d "${MEMORYWAM_BATTERY_TEXT_CACHE}" ]] || die "Text cache directory not found: ${MEMORYWAM_BATTERY_TEXT_CACHE}"
find -L "${MEMORYWAM_BATTERY_TEXT_CACHE}" -type f -size +0c -print -quit | grep -q . || die "Text cache contains no nonempty files: ${MEMORYWAM_BATTERY_TEXT_CACHE}"

episode_count="$("${runtime_bin}/python" - "${build_manifest}" "${rgb_contract}" "${RMBENCH_BATTERY_LEROBOT}/meta/info.json" "${latent_manifest}" <<'PY'
import json
import sys
...
if [[ -z "${resume_state}" ]]; then
  [[ ! -e "${output_dir}" ]] || die "Refusing to overwrite existing formal output: ${output_dir}"
else
...
cd "${repo_root}"
resolved_config="$("${runtime_bin}/python" scripts/train.py task=rmbench_battery_native_cache_25k --cfg job)"
...
expected = {
    ...
    "model.native_cache.memory_tokens": 32,
...
print("layerwise\t32\t4\t2\t4\tfalse\t1\t8")
')"
...
echo "task=rmbench_battery_native_cache_25k"
echo "dataset=${RMBENCH_BATTERY_LEROBOT}"
echo "episodes=${episode_count}"
echo "max_steps=25000"
echo "save_every=5000"
echo "save_steps=1000"
echo "checkpoint_steps=1000,5000,10000,15000,20000,25000"
...
accelerate launch \
  ...
  --num_processes 8 \
  scripts/train.py \
  task=rmbench_battery_native_cache_25k \
  "${resume_args[@]}"
