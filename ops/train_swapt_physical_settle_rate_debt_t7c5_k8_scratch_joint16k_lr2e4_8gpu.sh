...
output_dir=${OUTPUT_DIR:-/mnt/vepfs01/output/kevin.wang/memorywam/train/physical_settle_t7c5_k8_swapt_scratch_joint16k_lr2e4_seed42_v1}
...
task_name=rmbench_swapt_physical_settle_rate_debt_k8_scratch_joint_16k_lr2e4
...
import torch
s=PhysicalSettleRateManifestStore(sys.argv[1],expected_episode_count=50,expected_task="swap_T")
...
assert layout.retained_clean_tokens == 22  # anchor2 + K8 + five-frame open tail
...
PY
echo "task=swap_T method=physical_settle_rate_debt_t7c5_l4to8_k8 scratch=true joint=true lr=2e-4 steps=16000 save_every=2000 processes=${num_processes} grad_accum=${gradient_accumulation_steps} global_batch=8 manifest=${summary} output=${output_dir}"
