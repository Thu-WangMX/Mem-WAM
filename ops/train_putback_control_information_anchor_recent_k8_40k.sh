...
output_dir="${OUTPUT_DIR:-/mnt/vepfs02/output/kevin.wang/memorywam/train/control_information_v4_anchor_recent_k8_putback_e2e_40k_seed42}"
...
export CONTROL_INFORMATION_ANCHOR_RECENT_OUTPUT="${output_dir}"
...
assert len(lengths) >= 2 and min(lengths) >= 1 and max(lengths) <= 8
...
assert layout.retained_clean_tokens == 28

print("implementation=dynamic_k8_anchor2_recent4_v4")
...
resolved_config="$("${runtime_bin}/python" scripts/train.py task=rmbench_putback_control_information_anchor_recent_k8_40k --cfg job)"
...
echo "task=rmbench_putback_control_information_anchor_recent_k8_40k"
echo "max_steps=40000"
echo "memory_tokens=8 anchor_frames=2 recent_frames=4"
...
accelerate launch \
  ...
  --num_processes 8 \
  scripts/train.py task=rmbench_putback_control_information_anchor_recent_k8_40k
