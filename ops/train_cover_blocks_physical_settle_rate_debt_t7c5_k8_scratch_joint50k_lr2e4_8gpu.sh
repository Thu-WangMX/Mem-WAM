...
asset_root=${COVER_BLOCKS_ASSET_ROOT:-${memorywam_root}/data/memorywam_fullattention_cover_blocks_rgb_v3}
output_dir=${OUTPUT_DIR:-/mnt/vepfs01/output/kevin.wang/memorywam/train/physical_settle_t7c5_k8_cover_blocks_scratch_joint50k_lr2e4_seed42_v1}
manifest_root=${PHYSICAL_SETTLE_RATE_DEBT_MANIFEST:-/mnt/vepfs01/output/kevin.wang/memorywam/analysis/cover_blocks_physical_settle_rate_debt_t7c5_k8_v1}
task_name=rmbench_cover_blocks_physical_settle_rate_debt_k8_scratch_joint_50k_lr2e4
...
[[ -s ${manifest_root}/manifest.json ]] || die "Missing physical settle/rate-debt manifest"
...
export RMBENCH_COVER_BLOCKS_LEROBOT=${asset_root}/lerobot/cover_blocks
...
export COVER_BLOCKS_PHYSICAL_SETTLE_RATE_OUTPUT=${output_dir}
...
s=PhysicalSettleRateManifestStore(sys.argv[1],expected_episode_count=50,expected_task="cover_blocks")
...
PY
echo "task=cover_blocks method=physical_settle_rate_debt_t7c5_l4to8_k8 dynamic_raw_tail=true scratch=true joint=true lr=2e-4 steps=50000 save_steps=5k,10k,30k,40k,45k,50k processes=${num_processes} grad_accum=${gradient_accumulation_steps} global_batch=8 manifest=${summary} output=${output_dir}"
