...
output_dir="${OUTPUT_DIR:-/mnt/vepfs02/output/kevin_wang/memorywam/train/fastwam_memorywam_fullkv_putback_rgb_v3_b1x8_5k_seed42_20260731}"
...
for required in \
  ...
  if [[ ! -s "${required}" ]]; then
    ...
    echo "Run ops/prepare_putback_memorywam_fullkv_v3.sh first." >&2
    exit 2
  fi
done
