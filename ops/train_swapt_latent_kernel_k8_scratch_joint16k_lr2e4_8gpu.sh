...
output_dir=${OUTPUT_DIR:-/mnt/vepfs01/output/kevin.wang/memorywam/train/latent_kernel_k8_swapt_scratch_joint16k_lr2e4_seed42_v1}
manifest_root=${LATENT_KERNEL_REGIME_MANIFEST:-${memorywam_root}/analysis/trainfree_multires_latent_kernel_v1/swapT}
task_name=rmbench_swapt_latent_kernel_regime_k8_scratch_joint_16k_lr2e4
preflight_only=${PREFLIGHT_ONLY:-0}
require_idle_gpus=${REQUIRE_IDLE_GPUS:-1}
die(){ echo "ERROR: $*" >&2; exit 2; }

[[ -d ${repo_root} && -x ${runtime_bin}/python ]] || die "Missing repository or runtime"
[[ -s ${manifest_root}/manifest.json ]] || die "Missing latent manifest"
...
RESOLVED=${resolved} OUTPUT=${output_dir} MANIFEST=${manifest_root} "${runtime_bin}/python" - <<'PY' || die "Resolved contract mismatch"
import os
from omegaconf import OmegaConf
c=OmegaConf.create(os.environ["RESOLVED"])
e={"output_dir":os.environ["OUTPUT"],"initial_weights":None,"resume":None,"learning_rate":2e-4,"max_steps":16000,"save_every":2000,"batch_size":1,"num_gpus":8,"data.train.minimum_history_frames":1,"data.train.latent_kernel_manifest_path":os.environ["MANIFEST"],"data.train.manifest_task":"swap_T","native_cache_train_mode":"full","model.loss.lambda_video":1.0,"model.loss.lambda_action":1.0}
for k,v in e.items():
...
PY
echo "task=swap_T method=latent_kernel scratch=true joint=true lr=2e-4 steps=16000 save_every=2000 manifest_lengths=${summary} output=${output_dir}"
