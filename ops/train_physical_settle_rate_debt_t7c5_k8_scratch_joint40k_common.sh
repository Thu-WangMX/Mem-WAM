...
: "${PHYSICAL_TASK_ID:?PHYSICAL_TASK_ID is required}"
: "${PHYSICAL_TASK_CONFIG:?PHYSICAL_TASK_CONFIG is required}"
case "${PHYSICAL_TASK_ID}" in
  rearrange_blocks|press_button) ;;
  *) echo "ERROR: unsupported PHYSICAL_TASK_ID=${PHYSICAL_TASK_ID}" >&2; exit 2 ;;
esac
...
asset_root=${PHYSICAL_TASK_ASSET_ROOT:-${memorywam_root}/data/fastwam_fullkv_remaining8_rgb_v3/${PHYSICAL_TASK_ID}}
output_dir=${OUTPUT_DIR:-/mnt/vepfs01/output/kevin.wang/memorywam/train/physical_settle_t7c5_k8_${PHYSICAL_TASK_ID}_scratch_joint40k_lr2e4_seed42_v1}
manifest_root=${PHYSICAL_SETTLE_RATE_DEBT_MANIFEST:-/mnt/vepfs01/output/kevin.wang/memorywam/analysis/${PHYSICAL_TASK_ID}_physical_settle_rate_debt_t7c5_k8_v1}
...
export RMBENCH_PHYSICAL_TASK_LEROBOT=${asset_root}/lerobot/${PHYSICAL_TASK_ID}
...
for f in "${FASTWAM_ACTION_DIT_INIT}" "${RMBENCH_PHYSICAL_TASK_LEROBOT}/meta/info.json" "${MEMORYWAM_PHYSICAL_TASK_STATS}" "${MEMORYWAM_PHYSICAL_TASK_CONTINUOUS_LATENTS}/manifest.json"; do [[ -s ${f} ]] || die "Missing ${f}"; done
...
if [[ ! -e ${manifest_root} ]]; then
  ...
  "${runtime_bin}/python" scripts/build_physical_settle_rate_debt_manifest.py \
    --lerobot-root "${RMBENCH_PHYSICAL_TASK_LEROBOT}" \
    --output "${manifest_root}" \
    --task "${PHYSICAL_TASK_ID}" \
    --episodes 50 \
    --replan-stride 16
fi
[[ -s ${manifest_root}/manifest.json ]] || die "Missing or incomplete physical settle/rate-debt manifest"

summary=$("${runtime_bin}/python" - "${manifest_root}" "${PHYSICAL_TASK_ID}" <<'PY'
...
s=PhysicalSettleRateManifestStore(sys.argv[1],expected_episode_count=50,expected_task=sys.argv[2])
...
resolved=$("${runtime_bin}/python" scripts/train.py "task=${PHYSICAL_TASK_CONFIG}" "num_gpus=${num_processes}" "gradient_accumulation_steps=${gradient_accumulation_steps}" --cfg job --resolve)
RESOLVED=${resolved} OUTPUT=${output_dir} MANIFEST=${manifest_root} TASK_ID=${PHYSICAL_TASK_ID} NUM_PROCESSES=${num_processes} GRAD_ACCUM=${gradient_accumulation_steps} "${runtime_bin}/python" - <<'PY' || die "Resolved contract mismatch"
import os
from omegaconf import OmegaConf
c=OmegaConf.create(os.environ["RESOLVED"])
e={"output_dir":os.environ["OUTPUT"],"initial_weights":None,"resume":None,"learning_rate":2e-4,"max_steps":40000,"save_every":0,"save_steps":[5000,15000,20000,25000,30000,35000,40000],"save_training_state_every":40000,"batch_size":1,"num_gpus":int(os.environ["NUM_PROCESSES"]),"gradient_accumulation_steps":int(os.environ["GRAD_ACCUM"]),"data.train.minimum_history_frames":1,"data.train.physical_settle_manifest_path":os.environ["MANIFEST"],"data.train.manifest_task":os.environ["TASK_ID"],"native_cache_train_mode":"full","model.loss.lambda_video":1.0,"model.loss.lambda_action":1.0}
for k,v in e.items():
...
PY
echo "task=${PHYSICAL_TASK_ID} method=physical_settle_rate_debt_t7c5_l4to8_k8 dynamic_raw_tail=true scratch=true joint=true lr=2e-4 steps=40000 save_steps=5k,15k,20k,25k,30k,35k,40k processes=${num_processes} grad_accum=${gradient_accumulation_steps} global_batch=8 manifest=${summary} output=${output_dir}"
...
accelerate launch --config_file scripts/accelerate_configs/accelerate_zero2_ds.yaml --num_processes "${num_processes}" scripts/train.py "task=${PHYSICAL_TASK_CONFIG}" "num_gpus=${num_processes}" "gradient_accumulation_steps=${gradient_accumulation_steps}"
