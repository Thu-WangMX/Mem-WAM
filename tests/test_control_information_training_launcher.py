...
LAUNCHER = ROOT / "ops" / "train_putback_control_information_k8_40k.sh"
MANIFEST = Path(
...
def test_control_information_training_preflight_locks_v3_k8_contract(tmp_path):
    assert LAUNCHER.is_file(), "formal v3 control-information launcher is missing"
    ...
    for expected in (
        "task=rmbench_putback_control_information_k8_40k",
...
def test_control_information_launcher_uses_new_user_root_and_is_shell_valid():
    assert LAUNCHER.is_file(), "formal v3 control-information launcher is missing"
    ...
    assert (
        "/mnt/vepfs02/output/kevin_wang/memorywam/train/"
        "control_information_v3_k8_putback_e2e_40k_seed42"
    ) not in source
