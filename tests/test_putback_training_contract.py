...
LAUNCHER = REPO_ROOT / "ops" / "train_putback_native_cache_25k.sh"
...
def _preflight_env(tmp_path: Path) -> tuple[dict[str, str], Path]:
    asset_root = tmp_path / "rmbench_lerobot_v21_rgb_v3"
    dataset = asset_root / "put_back_block"
    stats = tmp_path / "stats" / "dataset_stats.json"
    text_cache = tmp_path / "text_cache"
    latents = tmp_path / "continuous_episode_stride16_v4"
    ...
    _write_json(
        ...
        {"color_contract": "simulator_rgb_preserved", "episodes_per_task": 50},
    )
    ...
    env.update(
        {
            ...
            "RMBENCH_PUTBACK_LEROBOT": str(dataset),
            "MEMORYWAM_PUTBACK_STATS": str(stats),
            "MEMORYWAM_PUTBACK_TEXT_CACHE": str(text_cache),
            "MEMORYWAM_PUTBACK_CONTINUOUS_LATENTS": str(latents),
...
def test_putback_fresh_preflight_reports_exact_training_contract(tmp_path: Path):
    """Catches H=5, wrong memory/checkpoint settings, or a preflight that starts training."""
    ...
    for expected in (
        "task=rmbench_putback_native_cache_25k",
        "episodes=50",
        "max_steps=25000",
...
def test_putback_fresh_preflight_refuses_existing_output(tmp_path: Path):
    """Catches accidental overwrite/restart of an existing formal run."""
    env, output_dir = _preflight_env(tmp_path)
    output_dir.mkdir()
...
def test_putback_resume_preflight_accepts_complete_state_in_matching_output(tmp_path: Path):
    """Catches resume launchers that reject a valid FSDP state or silently start fresh."""
    env, output_dir = _preflight_env(tmp_path)
...
def test_putback_resume_preflight_rejects_incomplete_state(tmp_path: Path):
