...
This repository extends FastWAM with interchangeable embodied-memory backends for RMBench experiments. It currently includes Anchor-Helios latent compression, the validated MemoryWAM-like layerwise memory path, and the full-history KV baseline. See [the experiment contract](docs/MEMORY_COMPARISON.md) for architecture, data, training, inference, and verification details.

Model weights, datasets, runtime environments, training runs, evaluation outputs, logs, and credentials are intentionally excluded from Git. Configure their locations through the launch-script environment variables.
