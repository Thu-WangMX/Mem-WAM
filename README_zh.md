...
本仓库在 FastWAM 上加入可切换的具身记忆方案，用于 RMBench 实验。目前包含 Anchor-Helios latent 压缩、已验证的 MemoryWAM-like 分层记忆路径，以及完整历史 KV baseline。架构、数据、训练、推理与验证约束见 [实验说明](docs/MEMORY_COMPARISON.md)。

Git 仓库不包含模型权重、数据集、运行环境、训练输出、评测结果、日志和凭证；相关路径通过启动脚本的环境变量配置。
